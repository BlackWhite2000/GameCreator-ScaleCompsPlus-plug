...
class CustomCommandParams_15001 {
    uiboxsizeDiyname_yyvhc: { uiID: number, compName:string, compID:string, varName:string }; // 选择组件
    uiboxsizeDiyXY_yyvhc: number; // = 1; 缩放倍率(XY)
    uiboxsizeXY_yyvhc: number; // = 0;  
    uiboxsizeDiyX_yyvhc: number; // = 1; 缩放倍率(X)
    uiboxsizeDiyY_yyvhc: number; // = 1; 缩放倍率(Y)
    uiboxtween_yyvhc: number; // = 0; 是否过渡
    uiboxtweenmm_yyvhc: number; // = 0; 毫秒
    uiboxtween2_yyvhc: string; // = "linearNone"; 过渡效果
}
/**
* 自定义指令 15002-缩放组件倍率plus
*/
class CustomCommandParams_15002 {
    select: { uiID: number, compName:string, compID:string, varName:string }; // 选择组件
    scaleType: number; // = 0;  
    scaleXY: number; // = 1; 缩放倍率(XY)
    scaleX: number; // = 1; 缩放倍率(X)
    scaleY: number; // = 1; 缩放倍率(Y)
    isTween: boolean; // = false; 是否过渡
    scaleXYType: number; // = 0;  
    scaleXType: number; // = 0;  
    scaleYType: number; // = 0;  
    scaleXYVar: number; // = 0; 缩放倍率(XY)
    scaleXVar: number; // = 0; 缩放倍率(X)
    scaleYVar: number; // = 0; 缩放倍率(Y)
    tweenTime: number; // = 500; 毫秒
    tweenTimeVar: number; // = 0; 毫秒
    tweenTimeType: number; // = 0;  
    tween: string; // = "{"transType":0,"loopType":0,"timeType":0,"timeUnit":1,"totalTime":2,"tweenType":0,"tweenTypeName":"linearNone","curveData":[[0,0,0,99,1,1,1,3],[0,100,100]],"refreshInterval":16}"; 过渡方式
}
/**
* 自定义指令 15003-游戏静音
*/
